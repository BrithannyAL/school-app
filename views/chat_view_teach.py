...
def Chat_view_chat(page: ft.Page, params: Params, basket: Basket):
    ...
    supervisor_id = int(params.get('supervisor_id'))
    ...
    def load_messages():
        ...
        for message in messages:
            is_supervisor = message['id_sender'] == supervisor_id
            alignment = ft.alignment.center_left if is_supervisor else ft.alignment.center_right
            bg_color = "#E0E0E0" if is_supervisor else "#D1C4E9"
            sender = "Supervisor" if is_supervisor else "You"
    ...
    def send_message_callback(e):
        send_message(teacher_id, supervisor_id, message_input.value)
        message_input.value = ""
    ...
    user = get_user(supervisor_id)

    return ft.View(
        f"/teacher_chat/chat_view/{teacher_id}/{supervisor_id}",
        controls=[
            ft.AppBar(
                title=ft.Text(user['full_name'], weight=ft.FontWeight.BOLD, size=12),
                bgcolor=BG,
                leading=ft.IconButton(icon=ft.icons.ARROW_BACK, on_click=lambda e: page.go(f"/teacher_chat/{teacher_id}")),
